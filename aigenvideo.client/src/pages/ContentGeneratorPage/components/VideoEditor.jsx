                            ...
                            {scenesToDisplay.length > 0 && images.length > 0 && (
                                <div className="bg-blue-900/20 p-3 rounded-lg border border-blue-600/30">
                                    <p className="text-xs text-blue-300">
                                        💡 <strong>Tip:</strong> You can assign uploaded images to specific scenes using the dropdown that appears when you hover over an image.
                                        Scene images will be used for video creation instead of uploaded images.
                                    </p>
                                </div>
                            )}
                        </div>
                        ...
                        {videoProcessor.isProcessingVideo && (
                            <div className="mt-4">
                                ...
                                <div className="w-full bg-slate-700 rounded-full h-2.5">
                                    <div className="bg-blue-600 h-2.5 rounded-full transition-all duration-300" style={{ width: `${videoProcessor.progress}%` }}></div>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div className="flex items-center gap-2 bg-slate-800 rounded-xl mb-7 overflow-x-auto">
                    {['reference', 'customScenes', 'images', 'videoReview'].map((tab) => (
                        <button
                            ...
                            className={`text-xs font-medium text-white rounded px-2 py-1 transition-all whitespace-nowrap ${activeTab === tab ? 'bg-slate-700' : 'bg-slate-800 hover:bg-slate-700'
                                }`}
                        >
                            ...
                            >
                                {isLoading || videoProcessor.isProcessingVideo || isReviewingAudio ? (
                                    <>
                                        ...
                                        <span>{videoProcessor.isProcessingVideo ? 'Creating Video...' : isReviewingAudio ? 'Reviewing Audio...' : 'Processing...'}</span>
                                    </>
                                ) : (
                                    <>
                                        {activeTab === 'reference' ? (
